...
public class ArcProgress extends View {
	...
	private final int default_finished_color = Color.rgb(40,134,195);
//	private final int default_unfinished_color = Color.WHITE;
	private final int default_unfinished_color = Color.rgb(232,232,232);
...
//		default_text_size = Utils.sp2px(getResources(), 18);
	...
	protected void initByAttributes(TypedArray attributes) {
		...
		textColor = attributes.getColor(R.styleable.ArcProgress_arc_text_color,
				default_text_color);
		textSize = attributes.getDimension(
				R.styleable.ArcProgress_arc_text_size, default_text_size);
		...
		suffixText = TextUtils.isEmpty(attributes
				.getString(R.styleable.ArcProgress_arc_suffix_text)) ? default_suffix_text
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawArc(rectF, finishedStartAngle, finishedSweepAngle, false,
				paint);
...
//			此处画 进度 的位置
			canvas.drawText(text,
					(getWidth() - textPaint.measureText(text)) / 2.0f-10,
...
//			此处画的是 % 的位置
			canvas.drawText(suffixText,
					getWidth() / 2.0f + textPaint.measureText(text)
							+ suffixTextPadding-20, textBaseline + textHeight
							- suffixHeight, textPaint);
		}
